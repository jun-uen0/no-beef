...
- 手順：`npm run build` → `bash scripts/launch-test-chrome.sh` → `python3 -m http.server 8787 -d test/fixtures` → `node scripts/verify-fixture.mjs`(カバーの判定)・`node scripts/verify-rewrite.mjs`(言い換えの導線)。終わったら`close-cdp-chrome.sh`とサーバのkillで必ず片付ける
